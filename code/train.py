...
parser.add_argument('--rte', action='store_true',
                    help='means that the task is rte')
parser.add_argument('--debug', action='store_true',
                    help='whether to set the logging level to debug')
parser.add_argument('--answer_key', action='store_true',
                    help='whether to use the answer key')
parser.add_argument('--epochs', type=int, default=25,
                    help='number of epochs to train for')
parser.add_argument('--sample_size', type=int, default=16,
                    help='number of samples for attention')
parser.add_argument('--initial_temperature', type=float, default=10,
                    help='initial temperature of sampler')
parser.add_argument('--annealing_schedule', type=float, default=0.5,
                    help='proportion of steps to anneal the temperature')
...
parser.add_argument('--pooling_function', choices=['sigmoid','softmax'], default='sigmoid',
                    help='type of pooling function for statute model')
parser.add_argument('--word_embeddings', type=str, default=None,
                    help='path to word embeddings')
parser.add_argument('--use_tokenized_text', action='store_true',
                    help='whether to use text tokenized using law tokenizer')
...
if args.answer_key:
    data_files['statutes']=args.datadir.rstrip('/')+'/answer_key.txt'
    logging.info('using answer key')
if args.use_tokenized_text:
    data_files={'statutes': args.datadir.rstrip('/')+'/tokenized_statutes.txt',
            'train': args.datadir.rstrip('/')+'/tokenized_train.tsv',
            'test': args.datadir.rstrip('/')+'/tokenized_test.tsv'}
data={}
...
try:
    ...
    mean_tax_amount=85804.875
    stddev_tax_amount=66656549654.83437**0.5
...
# scratch that if it's RTE
if args.rte:
    logging.info('using rte data')
    data_files={'train': args.datadir.rstrip('/')+'/train_rte.tsv',
        'dev': args.datadir.rstrip('/')+'/dev_rte.tsv',
        'test': args.datadir.rstrip('/')+'/test_rte.tsv'}
...
def relative_error(output,target):
    ...
    denom=torch.max(0.1*target,5000*ones)
    diff=torch.abs(output-target)
    error=torch.div(diff,denom)
    return error

def hinge_loss(output,target):
    delta=relative_error(output,target)
    loss=torch.max(delta-1,torch.zeros_like(delta))
    return loss

loss_functions=(torch.nn.BCEWithLogitsLoss(reduction='none'), hinge_loss)
logging.info('model:\n'+str(model))
if args.gpu:
    model.cuda()
...
for ep in range(args.epochs):
    ...
    # anneal temperature exponentially to 0.1 in the first part of training, then set to 0
    alpha=float(ep)/(args.annealing_schedule*args.epochs)
    if alpha<=1:
        temperature=np.exp( (1-alpha)*np.log(args.initial_temperature) + alpha*np.log(0.1) )
    else:
        temperature=0
    def forward_pass(split):
        ...
        for position in range(0,len(data[split]),args.batch):
            ...
            if args.statutes:
                input_args.update({'statutes': data['statutes'], 'context': context, 'temperature': temperature})
            elif args.context:
            ...
            loss_numerical=loss_functions[1](logits,float_label_tensor)*output_is_numerical_tensor
            ...
            metrics['num_correct_numerical']+=sum([ 1 if ((l<1) and (o==1)) else 0 for l,o in zip(loss_numerical,output_is_numerical) ])
            ...
            if split=='train':
                ...
                if (metrics['num_examples_binary']+metrics['num_examples_numerical'])%(args.batch*args.update_period)==0:
                    ...
                    optimizer.step()
                    optimizer.zero_grad()
        # end for position in range(0,len(data[split]),args.batch)
    ...
    if (best_numerical_model_metrics['accuracy_numerical'],-best_numerical_model_metrics['average_loss_numerical'])\
            <(dev_metrics['accuracy_numerical'],-dev_metrics['average_loss_numerical']):
        best_numerical_model_metrics=dict(dev_metrics)
        save_numerical_model=True
...
def test_forward_pass(model_file):
    ...
    metrics={'num_correct_binary': 0, 'total_loss_binary':0, 'num_examples_binary':0,
            'num_correct_numerical': 0, 'total_loss_numerical':0, 'num_examples_numerical':0,
            'accuracy_samples':[], 'mse_samples':[], 'numerical_accuracy_samples':[]} # samples are for t-test
    ...
    for position in range(0,len(data['test']),args.batch):
        ...
        if args.gpu:
            ...
            output_is_numerical_tensor=output_is_numerical_tensor.to('cuda')
        loss_binary=loss_functions[0](logits,float_label_tensor)*(1-output_is_numerical_tensor)
        loss_numerical=relative_error(logits,float_label_tensor)*output_is_numerical_tensor
        predictions=[ (logits[ii]>0).cpu().float() if output_is_numerical[ii]==0 else logits[ii].cpu()
                for ii in range(logits.size(0)) ]
        predictions=torch.stack(predictions,dim=0)
        predictions=torch.max(torch.round(predictions),torch.zeros_like(predictions))
        ...
        for l,o in zip(loss_numerical,output_is_numerical):
            if o==1:
                metrics['numerical_accuracy_samples'].append( 1 if (l<1) else 0 )
        metrics['num_correct_binary']+=sum([ 1 if ((int(p)==l) and (o==0)) else 0 for p,l,o in zip(predictions,labels,output_is_numerical) ])
        metrics['total_loss_binary']+=sum([ l if o==0 else 0 for l,o in zip(loss_binary.cpu().tolist(),output_is_numerical) ])
        metrics['num_examples_binary']+=sum([ 1-x for x in output_is_numerical ])
        metrics['num_correct_numerical']+=sum([ 1 if ((l<1) and (o==1)) else 0 for l,o in zip(loss_numerical,output_is_numerical) ])
    ...
    metrics['confidence_interval']={'accuracy': t_test(metrics['accuracy_samples']),
            'mse': t_test(metrics['mse_samples']), 'numerical_accuracy': t_test(metrics['numerical_accuracy_samples'])}
    return metrics
# end define forward pass over data
# print results
...
logging_message='metrics test best numerical model ' \
        ...
        + ' confusion matrix (label,prediction) ' + str(numerical_metrics['confusion_matrix'])
logging.info(logging_message)
